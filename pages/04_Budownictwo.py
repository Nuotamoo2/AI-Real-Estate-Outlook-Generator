...
from utils import setup_session_state, load_css, render_sidebar, display_ai_section, prepare_data_for_chart_aggregate, clean_number_formatting, get_month_number, MONTH_ORDER, parse_float_safe, render_page_header
...
if st.session_state.data_loaded and st.session_state.gus_budowa:
    ...
    if ts is not None:
            ...
            for y in years:
                ...
                if not chart_year.empty:
                    ...
                    if cols_to_plot:
                        ...
                        lines_y, points_y = base_y.mark_line(point=True), base_y.mark_point().transform_filter(nearest_y)
        ...
        if bd["regional"] is not None:
            ...
            st.altair_chart(alt.Chart(df_melt).mark_bar().encode(x=alt.X('Obszar', sort='-y', title='Województwo'), y=alt.Y('Wartość', title='Liczba mieszkań'), color=alt.Color('Kategoria', title='Legenda'), tooltip=['Obszar', 'Kategoria', 'Wartość']).interactive(), use_container_width=True)
